...
### Phiên 1 tiếp theo — 2026-06-08 (sau test thực tế)
- Fix lỗi navigation: nút "Tiếp theo" ở step Creator không chuyển sang Input (`setStep(1)→2`)
- Fix lỗi navigation: nút "← Quay lại" ở step Keyword không về Input (`setStep(3)→2`)
- Fix lỗi navigation: nút "← Quay lại" ở step Timeline không về Keyword (`setStep(4)→3`)
- Rebrand: "FAM Trading Content Tool" → **"INC Content Tool"** (UI header + browser tab)

**Trạng thái hiện tại:** Tất cả navigation đúng, live tại https://content-tool-inc.vercel.app
